...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql = "DELETE FROM brands WHERE id=$userid";

    $sqlCheck = "SELECT id FROM products WHERE brandname = ?";
    ...
    if($stmt = $mysqli->prepare($sqlCheck)){
        ...
        if($stmt->execute()){
            ...
            if($stmt->num_rows >= 1){
                $message = "Cannot delete, this brand is already in used by one or more products.";
            } else{
                if ($mysqli->query($sql) === TRUE) {
                    $message = "Brand sucessfully deleted.";
                } else {
                    $message = "Error deleting the Brand. Please try again.";
                }
            }
        } else{
        ...
        <section class = "py-5">
            <div class = "container text-center">
                ...
                <div class = "add-to-db-wrapper">
                    <a class = "add-to-db-btn" href = "brands-backend.php">Go Back</a>
                </div>
            </div>
        </section>
</body>